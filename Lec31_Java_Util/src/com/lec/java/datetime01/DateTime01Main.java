...
public class DateTime01Main {
	...
	public static void main(String[] args) throws Exception {
		...
		// Date객체의 대부분의 메소드들은 현재 deprecated(앞으로 안 쓰게될 확률 높음) 됨.
		int year = now.getYear() + 1900; // 1900년 이후 경과 년도
		int month = now.getMonth() + 1; // 0부터 시작함 즉, 0이 1월이다
		int day = now.getDate();
		int weekday = now.getDay(); // 일요일이 0
		int hour = now.getHours();
		int minute = now.getMinutes();
		int second = now.getSeconds();
		System.out.println(year + "년 " + month + "월 " + day + "일 " + weekday + "요일 " + hour + "시 " + minute + "분 "
				+ second + "초 ");
		...
		long baseTime = System.currentTimeMillis(); // 현재시각을 ms로 리턴
		// 1970-01-01 00:00:00 UTC 기준으로 경과된 ms
//		System.out.println(baseTime);
		long curTime = baseTime + 2000; // 2초뒤의 시간값ms
