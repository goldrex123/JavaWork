...
--TO_CHAR 함수 (날짜 -> 문자)
SELECT TO_CHAR(SYSDATE, 'YYYY') 연도4자리,
...
FROM dual;
--#4301
SELECT SYSDATE, TO_CHAR(SYSDATE, 'YYYY-MM-DD HH24:MI:SS') AS 날짜 FROM DUAL;
...
SELECT 
	...
	TO_CHAR(1234.1234, '9999.99') "소수점 이하 2자리", --반올림 해줌
	to_char(123456789, '999,999,999') "천단위에 구분기호"
FROM DUAL ;
--#4303
SELECT NAME , TO_CHAR(NVL2((pay * 12) + BONUS ,(pay * 12) + BONUS, pay * 12), '99,999') AS 연봉 FROM T_PROFESSOR WHERE DEPTNO = 101;
...
SELECT 
	TO_DATE('2020-12-08', 'YYYY-MM-DD') TO_DATE 
FROM DUAL ;
--#4304
SELECT NAME , HIREDATE AS 입사일, PAY * 12 AS 연봉, TO_CHAR((PAY * 12) * 1.1, '99,999') AS 인상후 FROM T_PROFESSOR
