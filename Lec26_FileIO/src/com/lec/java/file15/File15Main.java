...
public class File15Main {
	...
	public static void main(String[] args) {
		...
		for (int i = 0; i < list.length; i++) {
			...
			System.out.println(list[i].length()); //length() '파일'의 크기(byte), '디렉토리'인 경우는 의미 없다
		}
		System.out.println();
		// 디렉토리의 내용 출력, enhanced for 문 이용
//		File tempDir = new File("temp"); //'상대경로' 이용한 File 객체 생성 
		
		//'절대경로'를 이용한 File 객체 생성
		String tempDirPath = System.getProperty("user.dir") + File.separator // 윈도우(\), linux & mac(/)	
				+ "temp";
