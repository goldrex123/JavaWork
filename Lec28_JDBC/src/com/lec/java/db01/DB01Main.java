...
public class DB01Main {
	...
	public static void main(String[] args) {
		...
		try {
			...
			// sql_insert = "INSERT INTO test_member(mb_no) VALUES(101)"; --> 에러!
...
//			sql_insert = "INSERT INTO test_member VALUES("+no+", '"+name + "', SYSDATE)";
			...
			// 7.ResultSet에 담겨있는 record를 확인
			// 7-1 컬럼 이름으로 받기
			System.out.println();
//			while(rs.next()) { //next() 레코드 하나 추출하고 true리턴, 더 이상 뽑아낼 레코드 없으면 false 리턴
...
//			
//			while(rs.next()) { //next() 레코드 하나 추출하고 true리턴, 더 이상 뽑아낼 레코드 없으면 false 리턴
//				String no = rs.getString(1); //getXXX() 에 인덱스 명시 (select에 명시한 열 순서 - 1부터 시작!)
...
//			}
			...
			// 7-3 null 처리하기
			// getXXX() 로 가져온 데이터가 NULL 값이면 null 을 리턴
//			System.out.println();
//			
//			while(rs.next()) { //next() 레코드 하나 추출하고 true리턴, 더 이상 뽑아낼 레코드 없으면 false 리턴
//				String no = rs.getString(1); //getXXX() 에 인덱스 명시 (select에 명시한 열 순서 - 1부터 시작!)
...
//			}
			...
			while(rs.next()) { //next() 레코드 하나 추출하고 true리턴, 더 이상 뽑아낼 레코드 없으면 false 리턴
				int no = rs.getInt("mb_no"); //getXXX() 에 인덱스 명시 (select에 명시한 열 순서 - 1부터 시작!)
				//mb_no 컬럼 값이 null이면 0리턴
				String name = rs.getString("mb_name");
				String birthdate = "";
				java.util.Date date = rs.getTimestamp("mb_birthdate");
				//java.sql.Date , java.sql.Time
				Date d = rs.getDate("mb_birthdate"); //날짜만 가져옵니다
				Time t = rs.getTime("mb_birthdate"); //시간만 가져옵니다
				
				if( d != null ) {
					birthdate = new SimpleDateFormat("yyyy년MM월dd일").format(d) + " " + new SimpleDateFormat("hh:mm:ss").format(t);
				}
				
				String result = no + "\t | " + name + "\t | " + date;
				System.out.println(result);
			}
