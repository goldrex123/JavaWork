...
public interface Pb {
	...
	// [동작 설계]
	// 1. 이름(name) 과 전화번호(phoneNum) 과 (email) 값이 주어지면 전화번호 데이터(PhonebookModel) 를
	// 생성하여 저장,
...
//	public static final int QUERY_FAIL = 0;
	...
	public static final int ERR_EMPTY_STINRG = 2;// 입력문자열이 empty (혹은 null)
	public static final int ERR_UID = 3;// uid가 없는 경우

	// 에러 문자열
	public static final String[] ERR_STR = { "일반 오류", "인덱스 오류", "문자열 오류", "UID 오류" };
	// 인덱스 범위 벗어남
	// 입력문자열이 empty (혹은 null)
	// uid가 없는 경우
	...
	public static final String SQL_INSERT = "INSERT INTO " + TBL_NAME + " (pb_uid, pb_name, pb_phonenum, pb_memo) VALUES ("
			+ SEQ_NAME + ".nextval, ?, ?, ?)";
	...
	public static final String SQL_DELETE_BY_UID = "DELETE FROM " + TBL_NAME + " WHERE " + COL_LABEL_UID + " = ?";
	
	public static final String SQL_MAX_BY_UID = "SELECT MAX(" + COL_LABEL_UID + ") FROM " + TBL_NAME;
	
	public static final String SQL_SELECT = "SELECT * FROM " + TBL_NAME + " WHERE " + COL_LABEL_UID + " = ?" ;
